document.addEventListener('DOMContentLoaded', () => {
    ...
    themeToggle?.addEventListener('click', () => {
        ...
        themeToggle.textContent = isDark ? 'Light' : 'Mode';
            ...
            try {
                ...
                if (data.message) {
                    ...
                    if (btn.classList.contains('btn-wishlist-toggle')) {
                        ...
                        btn.textContent = data.added ? 'In Wishlist' : 'Add to Wishlist';
                    }
    ...
    function displaySearchResults(products) {
        ...
        searchDropdown.innerHTML = products.map(p => `
            <a href="/product/${p.id}/" class="search-result-item">
                ${p.image ? `<img src="${p.image}" alt="">` : '<span class="search-placeholder">Product</span>'}
                <div class="result-info">
                    <span class="result-title">${p.name}</span>
                    <span class="result-price">Rs. ${p.price}</span>
                </div>
            </a>
        `).join('');
        searchDropdown.classList.add('show');
